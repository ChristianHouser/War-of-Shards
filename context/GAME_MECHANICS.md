...
- **Drag preview**: while dragging (Ranged or Melee, from the roster or repositioning an already-placed card), the element following the cursor/finger is a full card — same color gradient, icon, name, HP text, and special-hero glow as the placed card — not a stripped-down label. See `buildGhost()`.
- **Scroll vs. drag on the roster list (touch)**: the roster list scrolls vertically, and dragging a card to place it is also a mostly-vertical gesture, so on touch a quick swipe scrolls the list instead of grabbing a card — grabbing one requires a brief press-and-hold (`TOUCH_DRAG_HOLD_MS`, currently 250ms) with the finger roughly stationary (`TOUCH_DRAG_CANCEL_PX`, 10px) first. See `attachRosterDragStart()`. Mouse/pen start dragging instantly, same as always — there's no scroll gesture to protect against on desktop. Already-placed battlefield cards (Ranged slots, Melee column) are unaffected — that area doesn't scroll, so they keep instant drag-start on touch too.
