...
- **Two ways to place a hero: tap the "+" badge, or drag (mouse only).** Every roster card shows a small green "+" badge in its upper-right corner — tapping/clicking it auto-places that hero in the next open row from the top (Ranged: first empty of the `RANGED_SLOTS` slots; Melee: first Ranged-row index not already guarded by an *aligned* Melee — see `autoPlaceCharacter()`). This is the primary interaction on touch. Dragging a card is still available, but **mouse-only** (`e.pointerType !== 'mouse'` bails out of `startRangedDrag()`/`startMeleeDrag()` immediately) — kept for precise placement during testing, including the one thing "+" can't express: a Melee's half-step boundary position (see below). Touch/pen pointerdown on a card does nothing at all, so there's no conflict with scrolling the roster list.
- **Every placed card shows a small red "−" badge** in its upper-right corner — tapping/clicking it removes that hero and returns it to the roster (`unplaceRanged()`/`unplaceMelee()`), on any input.
- **Ranged placement**: the fixed `RANGED_SLOTS` (= `TEAM_CAP` = 5) grid — filled or empty ("tap + to place" placeholder) at fixed positions. "+" always fills the first empty slot; mouse-drag (a Ranged character from the roster, or an already-placed one) can drop on any specific empty slot instead — the player's choice of exact position, so gaps are allowed (e.g. slots 1 and 3 filled, others empty). Dropping on an already-occupied slot is rejected (no swap); dragging a placed card outside the battlefield removes it.
- **Melee placement**: no fixed slots of its own — every Melee card locks onto one of two positions relative to the (always-rendered, filled-or-not) Ranged slots: **level with one Ranged row**, or **on the half-step gap between two adjacent Ranged rows** (straddling both; boundary positions are drag-only, mouse-only — "+" only ever places aligned). Mouse-dragging a Melee character up and down live-previews this: the target row/gap lights up gold as you move, and any Ranged slot(s) it would actually protect (see below) additionally light up green. Releasing over the Melee column snaps the card into place there; releasing outside cancels the placement (or removes the hero, if it was already placed). Two Melee may never share the same row/half-step (see below) — "+" and drag both enforce this the same way.
- **Drag preview**: while mouse-dragging (Ranged or Melee, from the roster or repositioning an already-placed card), the element following the cursor is a full card — same color gradient, icon, name, HP text, and special-hero glow as the placed card — not a stripped-down label. See `buildGhost()`.
