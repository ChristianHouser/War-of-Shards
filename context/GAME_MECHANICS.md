...
- **Hidden during deployment**: the deployment screen no longer shows the enemy's real cards at all — a single "ENEMY ❓" placeholder column stands in for both enemy columns, so neither the characters nor which formation was drawn (and therefore not even the Ranged/Melee count split) leak before the player commits their own team. `ENEMY_TEAM` is decided once, at deployment-entry time, same as before — only the on-screen reveal is deferred to `renderBattlefield()` (called from `startBattle()`), which shows the real roster exactly as it always has.
